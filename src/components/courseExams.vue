<template>
    <div>
        <h3 v-if="exams.length == 0"> There is no exam in this course </h3>
      ...
      <template v-slot:default>
        ...
        <tbody>
          <tr
            ...
            :key="exam.id" 
          >
            <td> <span @click="viewTask(exam)" class="task-title"> {{ exam.title }} </span> </td>
            <td>{{ exam.published_date }}</td>
            <td> {{ exam.due_date }}</td>
          </tr>
        </tbody>
      </template>
        ...
        <v-btn v-if="user.role == 'instructor'" @click="createModal"> Add exam </v-btn>
    ...
    <modal name="createModal" height="auto"> 
      ...
      <v-btn :disabled = disable color = primary @click="addexam" > add exam </v-btn>
